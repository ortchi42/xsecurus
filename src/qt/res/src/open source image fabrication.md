Processing path for Securus H logo.

Primary source is vector serialisation in `securus-h-74ade8ff.svg`, exported as 1024 x 1024 `PNG` format to `securus-h-74ade8ff.png`

Copy `securus-h-74ade8ff.png` to `securus-1024x1024.png`, then ...
...
convert securus-1024x1024.png -resize ${b}x${b} bitcoin${b}.xpm
done
```
...
Create the png set by resizing `securus-1024x1024.png` according to the following specification, in which `x2` means "twice the size" - if `icon_16x16.png` is 16 pixels x 16 pixels, the `x2` file will the same image resized to 32 pixels (16 * 2) x 32 pixels (16 * 2):
